...
| [`valorant_feed.py`](../autostream/clips/valorant_feed.py) | Valorant, `mode: feedbar`. OCR reads the player's own name in only 13-16% of the frames it is on screen, so the feed's **coloured bars** are read instead: the game outlines your own half of a row in yellow, and which end it is on says kill or death. No OCR, no name, nothing to configure. Frames are piped straight from ffmpeg, dropped on the GPU before they are copied off it (a 120 fps recording sampled at 2 fps was downloading 118 frames in 120 for nothing), and spans are read four at a time: 15m05s -> 8m35s on 70 minutes of 1440p120, same output |
| [`feed_ocr.py`](../autostream/clips/feed_ocr.py) | Valorant's **second look**: where the colour reader is unsure, that row alone is re-read as text. Three doubt rules over what the reader already produced (a kill frame nothing counted, your yellow on a row called an assist, a yellow panel with no row at all), then OCR of the white name text in those slots only. Measured: 14 of 16 kills -> 16 of 16 on a 33-minute match for +55% scan time, where reading every frame is +585%. The name comes from `games.yaml` or is **learned** from rows already counted as yours, so Valorant still needs no setup. Optional: without Tesseract the colour reader's kills come back untouched |
| [`songfetch.py`](../autostream/clips/songfetch.py) | A reel's song from a YouTube link, for the Studio. `video_id()` accepts every shape of video link and refuses a playlist or a non-YouTube link before anything is fetched (given a playlist, yt-dlp fetched an arbitrary song from it). A `Fetch` reads the link first, refusing a live stream or anything over 20 minutes, then downloads the audio alone (m4a where offered) into `VIDEO_HOME/songs`, named by title and video id so the same link is reused, reporting progress for the page's loader. `explain()` turns yt-dlp's errors — measured: unavailable, DNS failure, refused connection, age restriction, live-stream recording — into what happened and what to do. yt-dlp is optional: without it the dialog says so and choosing a file works as before |
| [`killmark.py`](../autostream/clips/killmark.py) | Valorant's **kill emblem**, the ring the game draws low in the middle of the screen for every kill, whatever the skin. Found by how much of the edge energy on circles around its centre points radially. Screen-read kills move onto their emblem, a kill with none is dropped, and an emblem no kill claimed is added unless the **spectator card** (its thin left rule, not its words) was up — a dead player sees their team-mates' emblems. Measured against Riot's records: feed alone 67 of 76 kills and 15 phantoms; with the emblem 74 and 4. Never touches a kill from a match record, and scans only what no record covers |
...
| [`studio.py`](../autostream/clips/studio.py) | The **Studio**. `library()` lists every clip under the clips folder by game and run, with each kill placed inside its own file (a master starts at the clip's `start`); `delete_clips()` removes chosen clips and their verticals and drops their rows from `clips.json` — only what a run's manifest lists, with a dry run that names the reels using them. `plan(part=(start, end))` plans to a song part chosen **before** planning: reel zero is its start, every moment goes in unless the part is full, and a shortfall is filled by `_lengthen_run_ups()` (shortest run-up first, within each clip's footage, never back into footage another shot shows, never past `rulebook.MAX_LEAD_UP_SECONDS`) before the reel ends early with a note. Every plan records `selection` — each chosen clip and why any is not in the reel — plus its notes (`plan_notes`) and a `plan_sig`, so `derive()` can say which chosen clips are in the timeline now and whether it has been edited since. `plan()` turns chosen clips, a style and an optional song into a **project** whose shot lengths and run-ups are whole beats, so every cut and kill lands on the grid; reel zero is a beat of the song. Each shot's kill effects, transition, speed and camera are drawn by a seeded `random.Random` from the project's **pools** (the style's by default), never repeating the shot before, with white flashes kept at the reference rate; `vary()` draws again without moving a cut. `apply_song()` sets the part of the song — its start un-snapped, since 10 ms nudges are the point — then `apply_marks()` re-times the cuts so shot N's kill lands on mark N, and `fit_to_part()` drops the shots the part cannot hold. The mark is the fixed point: a clip with too little footage before its kill has its run-up **stretched** (`stretch_to()`: the last 1.5 s play as recorded, everything before them slowed to no less than `MIN_STRETCH`, then the opening frame held up to `MAX_HOLD`) rather than landing early; only past that does an opener become the lead-in. `normalise()` clamps anything the page sends — clips must be inside the clips folder, part ids must exist — and derives every time the timeline draws. A plan records the song hits its kills were put on as `song_marks` (song seconds, kept through `normalise()`), and `derive()` puts them on the reel's axis with the **drift** of each shot's kill from its own mark and how many are within `ON_MARK`, so the timeline's music lane can show whether the reel kept the promise its notes make. The render is **two stages**: each shot to its own file with its speed (one piecewise `setpts` map, so the kill source frame is on screen at the planned instant) and effects, cached by a hash of everything that went into it; then the join, where transitions overlap handles taken either side of the cut so each is centred on it, the game audio is laid shot by shot under the song — gated to the moments around each kill, with the music dipped just before every kill — and loudness is brought to −10 LUFS by gain into a peak limiter, measured twice. A freeze replaces footage rather than lengthening a shot, so nothing downstream moves. The project is saved beside the mp4 as `<name>.reel.json`, which is what reopens as the editable timeline |
| [`intros.py`](../autostream/clips/intros.py) | The **intro clip** a reel opens over — the user's own GIF or video, for songs whose lead-up to the drop is longer than any built-in intro effect can cover. `add()` copies the file into `VIDEO_HOME/intros` and re-encodes it to one normalised mp4 with a `<stem>.json` sidecar, which is what makes it scrubbable in a `<video>` (a GIF is not), previewable at all (a .mkv may not decode in the webview), reusable across reels (a path into Downloads is not), and free of codec special-cases at render time. `inside()` is the guard every door shares: the media route and `studio.normalise()` both refuse anything that is not really an mp4 in that folder. An intro is **not a shot** — it is joined in **front** of the finished reel, so the reel's length, cuts, kills and beat grid are untouched and nothing it was cut to have is covered; `intro_head_command` renders the trimmed clip to the reel's own size, rate and one stereo track (silence when the clip has none, because concat matches streams by position), and `StudioJob._prepend_intro` splices the two with the reel stream-copied, falling back to a re-encoded join if an encoder's headers will not splice. It used to be one `overlay` on the reel's first seconds instead, which covered whatever the reel opened with |
| [`facecam.py`](../autostream/clips/facecam.py) | The **facecam** in a reel. Two sources: `inset`, a box the user draws once round a camera OBS already composited into the recording (a fraction of the frame, remembered for every reel), and `file`, a separate camera video **linked** to the session it was recorded beside (or to one added clip) in `.studio/facecam.json`. Only a path the OS dialog returned is ever linked, which is what lets `studio.normalise()` hand a file outside the clips folder to ffmpeg -- the per-shot `cam_file` always comes from the links, never from the page. `sync()` lines a camera file up with a clip by cross-correlating the onset envelopes of their sound and refuses a peak it cannot tell from the rest. `layout()` turns the choices into pixel boxes: vertical **stack** (camera across the top, `split` of the height) or **corner** (anywhere, `size` of the width); a 16:9 reel is always corner. The renderer puts the camera through the shot's own time map, so a speed ramp or a freeze keeps the face with the kill, and kill effects touch the game only |
| [`uploads.py`](../autostream/clips/uploads.py) | **Clips the player brings.** A video file becomes a run folder of its own (`import-<date>-<name>`, hard-linked where it can be), because a run's kills are shared by its clips and an added clip's are its own. Kills come from the game's own detector (`detector()`, on a thread) or are marked by hand in the page; what is saved is what the reel is cut to. The title ("1v3 clutch") is the clip's name, not drawn on the video |
| [`rulebook.py`](../autostream/clips/rulebook.py) | The editor's rules the Studio planner runs on every reel, each with the measurement that produced it: `moments()` (a shot per kill, close kills together, a beat of run-up), `drop_still()` (footage around each kill measured by `studio.action`; no movement, no moment), `phrase_seconds()`/`select()`/`order()` (whole 8-bar phrases, the strongest moments, the best two thirds in, places and shapes alternated), `walk()` (shot lengths from the song's loudness, the position in the phrase, bar lines, a pace-scaled cap), `budget()`/`transitions()`/`climax()` (effect sizing, bright-effect spacing, ramp rationing, cuts chosen by phrase and place, one freeze), `saturation_trim()` (measured colour gain per grade), and the sound, intro-build and ending constants the renderer reads. Pure functions; no files, no ffmpeg |
| [`studio_refs.py`](../autostream/clips/studio_refs.py) | 26 popular montage edits (25 Valorant, one Call of Duty) and reel v9, measured frame by frame (cuts, white flashes, first shot, fades, beat alignment). Numbers only. A Studio style's pace and flash rate are the medians of the edits it names |
...
<VIDEO_HOME>/
    ...
    songs/                   songs downloaded for a reel (clips/songfetch.py)
    intros/                  intro clips, one normalised mp4 + .json each (clips/intros.py)
