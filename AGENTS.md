...
Working rules for Codex — and any other agent or contributor — on AutoStream.
...
- `.Codex/settings.local.json` — machine-specific permission grants.
...
`.Codex/settings.json` is the shared, checked-in configuration. Personal grants — and
anything that widens permissions — belong in `.Codex/settings.local.json`, which is
